    ...
    <article className="relative job-card rounded-4xl border border-outline/70 bg-surface shadow-[0_10px_30px_rgba(37,99,235,0.12)] transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl max-h-fit">
      <div className="flex flex-col gap-4 p-5 sm:p-6">
        <div className="flex flex-col gap-2 px-3">
          <span className="inline-flex rounded-full bg-white/10 py-2 px-2 text-xs font-semibold uppercase tracking-[0.24em] text-primary w-fit">
            {job_type || "Job"}
          </span>
        ...
        <p className="text-sm leading-6 text-slate-600 px-3 first-letter:capitalize">
          {summary}
        </p>

        <div className="flex flex-wrap items-center justify-between gap-3 pt-3">
          <span className="rounded-full bg-white/5 px-3 py-2 text-sm font-medium text-primary capitalize min-w-20 flex items-center justify-center">
            {clinic_name}
          </span>
          <Link
            href={`/jobs/${id}`}
