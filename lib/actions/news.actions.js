...
export const getAllNews = async ({ limit = 10, page = 1, title, genre }) => {
  ...
  if (error) return { message: "Something went wrong", status: error };
  return data;
};

export const getNewsPaginated = async ({ limit = 3, page = 1 } = {}) => {
  const supabase = await createSupabaseClient();
...
export const getUniqueGenre = async () => {
  ...
  if (error) return { message: "Failed to fetch genres", status: error };
